...
Deno.serve(async (req) => {
  ...
  const vendor = typeof body.vendor === "string" ? body.vendor.trim() : "manufacturer";
  const sale_id = typeof body.sale_id === "string" && body.sale_id.trim() ? body.sale_id.trim() : null;
  const order_id = typeof body.order_id === "string" ? body.order_id.trim() : null;
  const category = typeof body.category === "string" ? body.category.toLowerCase().trim() : "cogs";
  ...
  if (!external_id) {
    const hashInput = `${date}|${order_id ?? ""}|${sale_id ?? ""}|${vendor}|${amount}|${category}|${notes ?? ""}|${due_date ?? ""}`;
